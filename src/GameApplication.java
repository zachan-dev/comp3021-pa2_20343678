...
public class GameApplication extends Application {
    ...
    private class ComputerThread implements Runnable {
        ...
        public void run() {
            ...
            for (Minister minister : readyMinisters) {
                // TODO: implement a computer thread that makes random decisions DONE
                ...
                City neighbor = chooseOneRandomly(gameEngine.getMap().getNeighboringCities(city));

                Technology tech = chooseOneRandomly(player.getTechnologies());

                int troopNum = getRandomInt(city.getTroops());
                ...
                Platform.runLater(() -> {
                    ...
                    String msg;
                    try {
                        msg = gameEngine.processPlayerCommand(actionNum, player, minister, city, neighbor, tech, troopNum);
                    }
                    catch (TooPoorException e) {
                        msg = e.getMessage();
                    }

                    infoBar.writeLog(player, msg);
    ...
    public void beginPlayerTurn(Player player) {
        ...
        List<Minister> ministers = currentPlayer.getMinisters();
        for (int i = 0; i < ministers.size(); i++) {
            ministers.get(i).beginTurn();
        }

        boolean is_human = currentPlayer.isHuman();
    ...
    public void initHandlers() {
        ...
        /**
         * TODO: initialize all handlers DONE
        ...
        menu.setNewGameHandler(e -> newGameHandler());
        ...
        infoBar.setSaveButtonHandler(e -> savePlayers());
    }
    ...
    public void newGameHandler() {
        ...
        /**
         * TODO: complete the new game handler DONE
         *
        ...
        HBox root = new HBox();
        root.getChildren().addAll(gameCanvas, infoBar);
...
*/
    ...
    public void loadGameHandler() {
        GameMap gameMap = gameEngine.getMap();
        try {
            gameMap.loadPlayers("saved_players.txt");
            gameMap.loadMap("map.txt");
        } catch (IOException e) {
            System.out.println("Cannot load saved players file.");
            Platform.exit();
        }
        //GUI
        ...
        infoBar.writeLog(currentPlayer, "Successfully Loaded Previously Saved Game.");
    }
    ...
    public void savePlayers(){
        infoBar.writeLog(currentPlayer, "Saving the Game......");
        try {
            gameEngine.getMap().savePlayers("saved_players.txt");
            infoBar.writeLog(currentPlayer, "The game is saved!");
        } catch (IOException e) {
            infoBar.writeLog(currentPlayer, "Error: Failed to save the game");
        }
    }
