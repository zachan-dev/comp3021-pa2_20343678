...
public class GameMap {
    ...
    public void savePlayers(String filename) throws IOException {
        ...
        for (int i = 0; i < players.size(); i++) {
            ...
            writer.write(p.getName() + ' ' + p.getGold() + ' ' + p.getSciencePoint() + ' ' + p.getProductionPoint() +
                    ' ' + p.getCities().size() + ' ' + p.getMinisters().size() + ' ' + (p.isHuman()?"Human":"Computer") + '\n');
            for (int j = 0; j < p.getCities().size(); j++) {
                City c = p.getCities().get(j);
                writer.write(Integer.toString(c.getId()) + ' ' + c.getName() + ' ' +
                        c.getPopulation() + ' ' + c.getTroops() + ' ' + c.getCropYields() + '\n');
            }
            for (int j = 0; j < p.getMinisters().size(); j++) {
                Minister m = p.getMinisters().get(j);
                writer.write(m.getClass().getSimpleName() + ' ' + m.getIntelligence() + ' ' + m.getExperience() + ' ' + m.getLeadership() + '\n');
            }
            writer.write('\n');
        }
