...
public class GameCanvas extends StackPane {
    ...
    public GameCanvas(GameMap gameMap, InfoBar infoBar) {
        /**
         * TODO: construct a game canvas DONE
         *
         *
        ...
        double width = gameMap.getWidth() * CELL_WIDTH;
        double height = gameMap.getHeight() * CELL_HEIGHT;
        ...
        background = new Image("background.png", width, height, false, false);

        for (int i = 0; i < 5; i++) {
            getChildren().add(canvasLayers[i]);
        }
    ...
    private void drawCity(City city, Image flag) {
        ...
        /**
         * TODO: draw the icon, name and flag of a city DONE
         *
        ...
        Image image = city.getImage(CELL_WIDTH, CELL_HEIGHT);
        Cell location = toCanvasCoordinates(gameMap.getCityLocation(city));
        double x = location.getX();
        double y = location.getY();

        canvasLayers[ICON_LAYER].getGraphicsContext2D().drawImage(image, x, y);

        canvasLayers[TEXT_LAYER].getGraphicsContext2D().strokeText(city.getName(), x + flag.getWidth(), y);
    ...
    private void drawLinesBetweenCities(City a, City b) {
        ...
        /**
         * TODO: draw a line between two cities DONE
        ...
        Player a_Owner = gameMap.getCityOwner(a);
        Player b_Owner = gameMap.getCityOwner(b);
        ...
        gc.strokeLine(a_location.getX() + CELL_WIDTH/2, a_location.getY()+ CELL_HEIGHT/2, b_location.getX()+ CELL_WIDTH/2, b_location.getY()+ CELL_HEIGHT/2);
    }
