...
public class InfoBar extends VBox {
    ...
    public InfoBar(GameMap gameMap) {
        ...
        /**
         * TODO: construct the info bar UI DONE
        ...
        HBox infoGroup = new HBox();
        infoGroup.getChildren().add(playerName);
        infoGroup.getChildren().add(playerResources);

        Label c = new Label("Cities");
        Label m = new Label("Ministers");
        Label t = new Label("Technologies");
        Label n = new Label("Neighbors");
        Label l = new Label("Logs");
        ...
        troopSlider.valueProperty().addListener((ov, oldVal, newVal) -> {
            lbSlider.setText("The value of the slider is " + newVal.intValue());
        });

        Pane buttonGroup = createActionButtonGroup();

        getChildren().addAll(infoGroup, c, cityListView, m, ministerListView,
            t, technologyListView, n, neighborListView, lbSlider, troopSlider, buttonGroup, l, logListView);
    ...
    private void onSelectedCityChangeHandler(City city) {
        /**
         * TODO: complete the on selected city changed handler DONE
        ...
        List<City> neighborsList = gameMap.getNeighboringCities(city);
        neighbors.addAll(neighborsList);
    ...
    private void actionButtonHandler(int actionNumber) {
        /**
         * TODO: complete the action button handler DONE
        ...
        City target = neighborListView.getSelectionModel().getSelectedItem();
        Minister minister = ministerListView.getSelectionModel().getSelectedItem();
        Technology tech = technologyListView.getSelectionModel().getSelectedItem();

        int troopNum = (int) troopSlider.getValue();

        gameActionHandler.handle(actionNumber, minister, selected, target, tech, troopNum);

    }

    public void displayPlayer(Player player) {
        /**
         * TODO: display a player in this info bar DONE
    ...
    public void setDisableButtons(boolean isDisabled) {
        /**
         * TODO: disable/enable ALL buttons in this component DONE
         *
        ...
        for (int i = 0; i < buttons.size(); i++) {
            buttons.get(i).setDisable(isDisabled);
        }
    ...
    /**
     * TODO: Not Required Feature: Save Game
     *
     */
